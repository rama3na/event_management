...
function UserLoginStore({children}) {
    ...
    const loginAdminUser=(userCredObj)=>{
        
        axios.post("http://localhost:3500/admin-api/admin-login",userCredObj)
        .then((response)=>{
             if(response.data.message==='Success'){
                 //save token to local storage
                 console.log(response)
    ...
    return (
    <div> 
        <loginContext.Provider value={[currentUser,loginErr,userLoginStatus,AdminLoginStatus,loginUser,loginAdminUser,logoutUser]}>
            {children}
